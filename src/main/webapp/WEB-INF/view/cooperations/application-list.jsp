		...
		<div class="layui-body">
			<blockquote class="layui-elem-quote">
				<a href="${PATH}/admin/index"><i
					class="layui-icon layui-icon-layer"></i> 主页</a> / <a>合作申请中</a>
			</blockquote>
	...
	<div style="display: none;" id="detailModal" align="center">
		<div class="layui-card">
			...
			<form class="layui-form">
				<input type="hidden" name="coopEmail" id="coopEmailInput" /> <input
					type="hidden" name="cooperationId" id="coopIdInput" />
				<div class="layui-form-item">
					<label class="layui-form-label">通知内容</label>
					<div class="layui-input-block">
						<textarea name="adminHandle" required lay-verify="required"
							placeholder="请输入" class="layui-textarea"></textarea>
					</div>
				</div>
				<button class="layui-btn  layui-btn-normal" lay-submit
					lay-filter="investigation">考察中</button>
				<button class="layui-btn" lay-submit lay-filter="abnormal">未通过</button>
			</form>
		</div>
	</div>
  ...
  <div class="layui-btn-container">
	...
	<script>
		...
		function renderTb() {
			...
			layui.use([ 'table', 'util', 'form' ], function() {
				...
				table.render({
					elem : '#dataListTb',
					height : 500,
					url : '${PATH}/cooperation/getApplicationList',
					text : {
						none : '未找到数据'
					},
					id : "datListTbId",
		...
		layui.use([ 'form', 'layer' ], function() {
			...
			//考察中
			form.on('submit(investigation)', function(data) {
				console.log(data.field) //当前容器的全部表单字段，名值对形式：{name: value}
				var datas = data.field
				$.ajax({
					url : "${PATH}/cooperation/investigationCoop",
					method : "POST",
					data : datas,
					success : function(res) {
						if (res.code == 100) {
			...
			form.on('submit(abnormal)', function(data) {
				...
				var datas = data.field
				$.ajax({
					url : "${PATH}/cooperation/abnormalCoop",
					method : "POST",
					data : datas,
					success : function(res) {
						if (res.code == 100) {
